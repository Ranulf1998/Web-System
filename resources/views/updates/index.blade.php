            ...
            <div class="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.25)] backdrop-blur-xl">
                ...
                @if (!empty($otaInfo['applied_at']))
                    <p class="mt-3 text-xs text-zinc-500">
                        Last applied update: <span class="font-semibold text-zinc-700">{{ $otaInfo['current_version'] ?? $currentVersion }}</span>
                        at {{ \Illuminate\Support\Carbon::parse($otaInfo['applied_at'])->format('M j, Y g:i A') }}
                    </p>
                @endif
            </div>
                ...
                <div class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-zinc-600">Pick any published release and download that exact version.</p>
                        </div>
                                ...
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                    ...
                                    <div class="flex flex-wrap items-center gap-2">
                                        <form method="POST" action="{{ route('tenant.updates.apply') }}">
                                            ...
                                            <button type="submit" class="rounded-full bg-[color:var(--brand-primary)] px-3 py-2 text-xs font-medium text-white shadow-sm transition hover:opacity-95">
                                                Download
                                            </button>
                                        </form>
