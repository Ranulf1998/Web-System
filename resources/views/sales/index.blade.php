        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="text-sm text-gray-500 font-medium">Total Sales</div>
                    <div class="text-3xl font-bold text-gray-900 mt-2">₱{{ number_format((float) $totalSales, 2) }}</div>
                </div>
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="text-sm text-gray-500 font-medium">Total Orders</div>
                    <div class="text-3xl font-bold text-gray-900 mt-2">{{ number_format($totalOrders) }}</div>
                </div>
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="text-sm text-gray-500 font-medium">Most Purchased</div>
                    <div class="text-xl font-bold text-gray-900 mt-2">{{ $topProduct }}</div>
                    <div class="text-sm text-gray-500 mt-1">{{ number_format($topProductQty) }} sold</div>
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="mb-6 flex gap-3">
                    <a href="{{ route('sales.index', ['period' => 'all']) }}" class="px-4 py-2 rounded-lg border text-sm font-medium transition {{ $period === 'all' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                        All Time
                    </a>
                    <a href="{{ route('sales.index', ['period' => 'today']) }}" class="px-4 py-2 rounded-lg border text-sm font-medium transition {{ $period === 'today' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                        Today
                    </a>
                    <a href="{{ route('sales.index', ['period' => 'week']) }}" class="px-4 py-2 rounded-lg border text-sm font-medium transition {{ $period === 'week' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                        This Week
                    </a>
                    <a href="{{ route('sales.index', ['period' => 'month']) }}" class="px-4 py-2 rounded-lg border text-sm font-medium transition {{ $period === 'month' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                        This Month
                    </a>
                </div>
